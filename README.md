...
4. Right pane tabs: **Levels** (Level hold + pack quotas), **Tiles** (Tile Actions + the selected kind, pack, start, end, stairs, HUD type, outcome **Spinner**, **Audio / Image / Video**, single-tile / multi-tile on a room), **Packs**, **Spinners**, **Players**, and **Start**. Tabs wrap. There is no unused Title field. Preview is a popup, not an inline pane. Tile Actions **Start tile** / **End tile** stay outline unless that cell is already start or end (then secondary/white). **Clear** (confirm: *Clear this tile?*) is hidden on HUD. Make/Convert type buttons are gone — the palette still places Stair/Room.
...
- **Tile Actions Audio / Image / Video:** corridor, stair, and room may hold any combination (independent rows). Room sound plays on **Enter** for a single-tile room, or when you stop on an interior tile. Crossing a tile or stair without stopping does not play. HUD cells do not hold attachments.
