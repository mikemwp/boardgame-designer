...
5. **Packs** can exist with zero cards — no CSV required. Tile Actions can attach that pack to a corridor tile or a **room**. Landing on a room offers **Enter** or **Pass**. Pass stays on the corridor. Enter on a single-tile room shows that tile’s media; Enter on a multi-tile room walks the interior until **Leave** on the entrance. **New card** edits title, body, **Timer seconds**, **Extra button**, **Audio**, and an optional outcome **Spinner**. Mark **End room** on a room with the existing End control. **Level hold** on the selected floor turns on per-pack reveal quotas before that floor’s stairs open.
